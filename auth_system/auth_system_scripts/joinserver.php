...
$query = "Update \"$db_table\" SET \"$db_columnServer\"='$serverid' Where \"$db_columnSesId\"='$sessionid' And \"$db_columnUser\"='$user'".($hash_enable && $hash_enable_timeout ? " and \"md5time\"::timestamp >= (now()-interval '$hash_timeout seconds')::timestamp" : "");
$result = pg_query($link, $query)
            or die ("Database error.");
    ...
    if(pg_affected_rows($result) == 1){
        ...
        echo "Bad login".($hash_enable && $hash_enable_timeout ? " or check timeout exceeded" : "");
    }
}
?>